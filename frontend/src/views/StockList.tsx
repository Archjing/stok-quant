...
export default function StockList() {
  ...
  const loadKlineData = async (symbol: string, period: KlinePeriod) => {
    ...
    try {
      const res = (await getStockKline(symbol, {
        period,
        years: period === "yearly" ? 10 : 5,
        market,
      })) as any;
        ...
        custom: ({ seriesIndex, dataPointIndex, w }: any) => {
          ...
          return `<div style="padding:4px 8px;font-size:12px">
            <div><strong>${dateStr}</strong></div>
                        <div>O: ${formatMoney(o)} H: ${formatMoney(h)}</div>
            <div>L: ${formatMoney(l)} C: ${formatMoney(c)}</div>
                ...
                {stocks.map((s: any) => (
                  <tr
                    ...
                    style={{
                      cursor: "pointer",
                      background:
                        selected === s.symbol ? "var(--bg-active)" : undefined,
                    }}
                  >
                    <td
                      className="mono"
                    ...
                    <td
                      style={{
                        ...
                        whiteSpace: "nowrap",
                        color:
                          selected === s.symbol ? "var(--selected)" : undefined,
                      }}
                    >
                      {s.name || "-"}
                    </td>
          ...
          >
            ...
            >
              ...
              <div className="stat-card">
                ...
                <div className="stat-value">
                  {daily.length > 0
                    ? (daily[daily.length - 1]?.volume / 1e6).toFixed(1) + "M"
                    : "-"}
                </div>
              </div>
              <div className="stat-card">
              ...
              >
                ...
                <div style={titleContainerStyle}>
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setChartType(chartType === "table" ? "kline" : "table");
                    }}
                  >
                    <span
                      style={
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  {chartType === "kline" && (
                    <div style={periodGroupStyle}>
                      {(["daily", "monthly", "yearly"] as KlinePeriod[]).map(
                        (p) => (
                          <button
                            key={p}
                  ...
                  <tbody>
                    {daily.slice(0, 30).map((d: any, i: number) => (
                      <tr key={i}>
                        ...
                        <td className="mono">{formatMoney(d.open)}</td>
                        <td className="mono">{formatMoney(d.high)}</td>
                        <td className="mono">{formatMoney(d.low)}</td>
