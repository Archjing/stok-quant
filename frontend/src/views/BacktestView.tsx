...
export default function BacktestView() {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        const data = (await getStockFilters({ market })) as any;
  ...
  const getStrategyName = (id: string) => {
    return isZh
      ? STRATEGIES[id as keyof typeof STRATEGIES]?.zh
      : STRATEGIES[id as keyof typeof STRATEGIES]?.en || id;
  };
  ...
  const MetricCard = ({ label, value }: { label: string; value: string }) => (
    <div className="stat-card">
      ...
      <div
        className={`stat-value ${value.startsWith("-") ? "negative" : "positive"}`}
      >
        {value}
      </div>
    </div>
          ...
          >
            ...
            >
              ...
              >
                ...
                {(filterType === "exchange" || filterType === "board") && (
                  <div className="form-group" style={{ minWidth: 100 }}>
                    <label className="form-label">&nbsp;</label>
                    <select
                      className="form-select"
                ...
                <div className="form-group">
                  ...
                  <button
                    ...
                    disabled={
                      !filterType ||
                      (filterType !== "custom" && !filterValue) ||
                      (filterType === "custom" && !customSymbols.trim())
                    }
                  >
            ...
            >
              ...
              >
                <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  {t("backtest.selectedStock") || "Selected Stock"}:{" "}
                </span>
                <span
                  style={{
                    fontSize: 16,
              ...
              >
                <div className="form-group" style={{ minWidth: 160 }}>
                  ...
                  >
                    <option value="sma_crossover">
                      {getStrategyName("sma_crossover")}
                    </option>
                    <option value="rsi_mean_reversion">
                      {getStrategyName("rsi_mean_reversion")}
                    </option>
                    <option value="macd">{getStrategyName("macd")}</option>
                    <option value="buy_and_hold">
                      {getStrategyName("buy_and_hold")}
                    </option>
                  </select>
                </div>
              ...
              <div className="card table-card" style={{ marginBottom: 12 }}>
                <div className="card-header">
                  <div className="card-title">
                    {t("backtest.strategyComparison")} - {compareResult.symbol}
                  </div>
                </div>
                  ...
                  <table className="data-table">
                    ...
                    <tbody>
                      {compareResult.strategies &&
                        Object.entries(compareResult.strategies).map(
                          ([id, s]: [string, any]) => (
                            <tr key={id}>
                              ...
                              <td
                                className={`mono ${s.total_return_pct > 0 ? "metric-positive" : "metric-negative"}`}
                              >
                                {formatPct(s.total_return_pct)}
                              </td>
                              <td
                                className={`mono ${s.sharpe_ratio > 1 ? "metric-positive" : "metric-neutral"}`}
                              >
                                {formatRatio(s.sharpe_ratio)}
                              </td>
                              <td className="mono metric-negative">
            ...
            {result && result.results && (
              ...
              >
                <MetricCard
                  ...
                  value={formatPct(result.results.total_return_pct)}
                />
                <MetricCard
                  label={t("backtest.annualizedReturn")}
                  value={formatPct(result.results.annualized_return)}
                />
                <MetricCard
                  label={t("backtest.sharpeRatio")}
                  value={formatRatio(result.results.sharpe_ratio)}
                />
                <MetricCard
                  label={t("backtest.sortinoRatio")}
                  value={formatRatio(result.results.sortino_ratio)}
                />
                <MetricCard
                  label={t("backtest.calmarRatio")}
                  value={formatRatio(result.results.calmar_ratio)}
                />
                <MetricCard
                  label={t("backtest.maxDrawdown")}
                  value={formatPct(result.results.max_drawdown_pct)}
                />
                <MetricCard
                  label={t("backtest.volatility")}
                  value={formatPct(result.results.volatility)}
                />
                <MetricCard
                  label={t("backtest.winRate")}
                  value={formatPct(result.results.win_rate)}
                />
                <MetricCard
                  label={t("backtest.totalTrades")}
                  value={String(result.results.total_trades || 0)}
                />
                <MetricCard
                  label={t("backtest.endCash")}
                  value={formatMoney(result.results.end_cash || 0)}
                />
              </div>
            )}
          </div>
                  ...
                  <tbody>
                    {(result.trades || []).map((t: any, i: number) => (
                      <tr key={i}>
                        ...
                        <td className="mono">{formatMoney(t.price || 0)}</td>

                        <td
                          className={`mono ${t.pnl > 0 ? "metric-positive" : t.pnl < 0 ? "metric-negative" : ""}`}
                        >
