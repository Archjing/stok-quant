export default {
  app: {
    title: "多市场量化",
    subtitle: "股票分析与回测",
    version: "v1.0.0",
  },
  nav: {
    navigation: "导航",
    dashboard: "仪表盘",
    stocks: "股票",
    backtest: "回测",
    dataSync: "数据同步",
    strategies: "策略",
    analysis: "分析",
  },
  dataSync: {
    title: "数据同步",
    summary: "点击按钮即可为 US/CN/HK 市场下载或增量更新数据。",
    selectMarket: "选择市场",
    currentStatus: "当前同步状态",
    market: "市场",
    syncStatus: "同步状态",
    currency: "币种",
    btnDownload: "历史数据下载",
    btnUpdate: "增量更新",
    btnRefreshPrices: "刷新价格",
    btnRefreshSymbols: "刷新股票池",
    runningTip: "后台同步任务正在运行，状态会自动刷新。",
    idleTip: "使用按钮触发下载或增量更新。",
    overviewTitle: "市场同步概览",
    overviewSubtitle: "所选市场的最新同步摘要。",
    tableSymbol: "代码",
    tableStatus: "状态",
    tableLastSync: "最后同步",
    tableRowCount: "行数",
    empty: "暂无同步概览数据。",
    actionSubmitted: "操作已提交成功。",
    requestFailed: "请求失败。",
    statusRunning: "运行中",
    statusIdle: "空闲",
  },
  dashboard: {
        title: "仪表盘",
    totalStocks: "股票总数",
    usStocks: "股票数量",

    strategies: "策略数量",
    dataSource: "数据源",
    lastUpdate: "最后更新",
    quickStart: "快速开始",
    browseStocks: "浏览股票",
    browseStocksDesc: "搜索和浏览股票数据",
    runBacktest: "运行回测",
    runBacktestDesc: "测试策略历史表现",
    viewStrategies: "查看策略",
    viewStrategiesDesc: "探索量化交易策略",
    technicalAnalysis: "技术分析",
    technicalAnalysisDesc: "RSI、MACD、布林带",
    systemInfo: "系统信息",
    backend: "后端",
    backtestingEngine: "回测引擎",
    dataSourceFull: "数据源",
    supportedExchanges: "支持交易所",
    uiFramework: "前端框架",
    clojureDsl: "Clojure DSL",
  },
  stocks: {
    title: "股票列表",
    loading: "加载股票中...",
    symbol: "代码",
    name: "名称",
    price: "价格",
    selectStock: "选择股票",
    selectStockDesc: "点击股票代码查看价格历史和技术指标",
    close: "收盘价",
    volume: "成交量",
    dataRange: "数据区间",
    recentPriceHistory: "近期价格走势",
    klineChart: "K线图",
    lastDays: "最近60天",
    date: "日期",
    open: "开盘",
    high: "最高",
    low: "最低",
    rsi: "RSI(14)",
    sma: "SMA(20)",
    searchPlaceholder: "输入股票代码或名称筛选（如 AAPL / Apple）",
    noMatch: "没有匹配的股票，请尝试其他代码或名称",
  },

  backtest: {
    title: "回测引擎",
    symbol: "股票代码",
    strategy: "策略",
    years: "年数",
    runBacktest: "运行回测",
    running: "运行中...",
    compareAll: "对比全部",
    singleStrategy: "单一策略",
    strategyComparison: "策略对比",
    return: "收益率",
    sharpe: "夏普率",
    maxDD: "最大回撤",
    trades: "交易次数",
    totalReturn: "总收益率",
    annualizedReturn: "年化收益率",
    sharpeRatio: "夏普比率",
    sortinoRatio: "索提诺比率",
    calmarRatio: "卡玛比率",
    maxDrawdown: "最大回撤",
    volatility: "波动率",
    winRate: "胜率",
    totalTrades: "总交易次数",
    endCash: "最终资金",
    tradeLog: "交易记录",
    tradeCount: "笔交易",
    side: "方向",
    qty: "数量",
    price: "价格",
    pnl: "盈亏",
    tag: "标签",
    runBacktestPrompt: "运行回测",
    runBacktestPromptDesc: '选择股票代码和策略，然后点击"运行回测"查看结果',
    // Filter
    stockFilter: "选股筛选",
    strategyControl: "策略控制",
    selectedStock: "已选股票",
    allStocks: "全部股票",
    bySector: "按板块",
    byExchange: "按交易所",
    byBoard: "按板块/市场层级",
    byMarketCap: "按市值",
    byIndex: "按指数",
    custom: "自定义",
    selectSector: "选择板块",
    selectExchange: "选择交易所",
    selectBoard: "选择板块/层级",
    selectTop: "选择Top",
    selectIndex: "选择指数",
    customPlaceholder: "AAPL,MSFT,GOOGL",
    stocks: "只股票",
  },
  strategies: {
    title: "策略",
  },
  analysis: {
    title: "技术分析",
    loadAnalysis: "加载分析",
    currentPrice: "当前价格",
    signal: "信号",
    bullish: "多头",
    bearish: "空头",
    rsiStatus: "RSI状态",
    overbought: "超买",
    oversold: "超卖",
    neutral: "中性",
    technicalIndicators: "技术指标",
    lastUpdated: "最后更新",
    recentData: "近期数据",
    loadAnalysisData: "加载分析数据",
    loadAnalysisDataDesc: '选择股票代码并点击"加载分析"查看技术指标',
  },
  common: {
    loading: "加载中...",
    applyFilter: "筛选",
    market: "市场",
    noData: "暂无数据",
    clear: "清空",
  },
};
