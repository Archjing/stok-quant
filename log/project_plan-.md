...
用户(浏览器) → Nginx(80) → Frontend(React + Vite)
                         → Backend(FastAPI:8777) → yfinance API
                                                  → SQLite(本地存储)
                                                  → Clojure回测引擎
```
...
analysis:
  default_indicators:
    [sma_20, sma_50, sma_200, ema_12, ema_26, macd, rsi_14, bollinger_20]
  default_filters:
    { market_cap: { min: 1e9 }, price: { min: 5.0 }, volume: { min: 100000 } }
```
...
- **backend**: python:3.12-slim (端口8777)
- 网络: us-stock-network
- 持久化: `./data` → `/app/data`
...
- python:3.12-slim → uv sync → uv run uvicorn 启动

nginx.conf 反向代理：

- `/` → 静态文件
- `/api/` → proxy_pass backend:8777

### 3.5 本地启动脚本

**backend/start.bat** — FastAPI + Uvicorn (8777端口, 热重载)
...
_文档生成时间: 2026-05-07_
_基于对 014_stock-us 项目全部代码的分析整理_
