...
- **前端数据同步页面**：新增 Data Sync 页面，可在界面中手动触发多市场历史数据下载、增量更新和价格刷新
...
├── frontend/                   # React 前端应用
│   ├── src/api/                # API 调用封装
│   ├── src/views/              # 页面视图（股票、回测、分析、策略、数据同步等）
│   ├── src/components/         # 通用组件
│   ├── src/i18n/               # 国际化资源
│   ├── src/stores/             # 状态管理
│   └── src/styles/             # 样式文件
...
- 支持日线历史数据和 K 线图数据输出
- 支持财务信息查询
- 支持通过后端接口触发历史数据下载、增量更新和价格刷新
- 新增前端 **Data Sync** 页面，可直接在导航栏进入，用于选择市场并手动调用后端数据同步能力
- Data Sync 页面支持查看当前同步状态、运行状态、币种、每只股票的同步状态、最后同步时间和数据行数
...
以下接口已接入前端 **Data Sync** 页面，可在侧边导航栏点击“Data Sync / 数据同步”直接调用：

- `GET /api/data/status?market=US|CN|HK`：查看同步状态和市场同步概览
- `POST /api/data/download?market=US|CN|HK`：触发历史数据全量下载
- `POST /api/data/update?market=US|CN|HK`：触发增量更新
- `POST /api/data/refresh-prices?market=US|CN|HK`：刷新实时/最新价格

## 前端页面

当前前端侧边导航包含：

- **Dashboard**：项目概览、快速入口和系统信息
- **Stocks**：多市场股票列表、搜索、K 线和近期行情查看
- **Backtest**：单策略回测与多策略对比
- **Data Sync**：手动触发后端数据抓取/同步能力，支持选择 US/CN/HK 市场并执行历史数据下载、增量更新和价格刷新
- **Strategies**：策略信息与扩展入口
- **Analysis**：技术指标分析页面
...
3. **手动同步数据**：在侧边栏进入 Data Sync 页面，选择 US / CN / HK 市场后，可点击历史数据下载、增量更新或刷新价格
4. **技术分析**：在 Analysis 页面查看技术指标和近期行情数据
5. **运行回测**：在 Backtest 页面选择股票、策略和回溯年数后执行回测
6. **策略对比**：启用 Compare 模式，对多个策略进行横向比较
7. **数据预热**：如某只股票本地无缓存，系统可在首次请求时自动下载数据；也可通过 Data Sync 页面提前触发市场级数据同步
8. **查看文档**：通过 `/docs` 查看 Swagger API 文档
